    ...
    LV.Columns.Add("英文", 100);
        ...
        void GetData()
        {
            ...
            while (adapter.Read())
            {
                ...
                item.SubItems.Add(adapter["C"].ToString());
                item.SubItems.Add(adapter["M"].ToString());
                item.SubItems.Add(adapter["E"].ToString());
                LV.Items.Add(item);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if(LV.SelectedItems.Count > 0) 
            {
                ...
                SqlConnection connection = new SqlConnection(@"Data Source = localhost; Initial Catalog = score; Integrated Security=True; Connect Timeout=30; Encrypt=False;");
                connection.Open();
                SqlCommand cmd = new SqlCommand("DELETE FROM score_t WHERE C = " + Int32.Parse(selected[0].SubItems[1].Text), connection);
                cmd.ExecuteNonQuery();
                connection.Close();
                MessageBox.Show("刪除成功");
